...
class KGC_UKAN(nn.Module):
    ...
    def forward(self, x, return_uncertainty=False):
        ...
        ds = []
        unc = []   # collected boundary-distance uncertainty maps (deep -> shallow)
        out = F.relu(F.interpolate(self.decoder1(out), scale_factor=2, mode='bilinear', align_corners=False))
        if self.use_pulask:
            r, u = self.skip4(t4, out); out = out + r; unc.append(u)
        else:
            out = out + t4
        ...
        if self.use_pulask:
            r, u = self.skip3(t3, out); out = out + r; unc.append(u)
        else:
            out = out + t3
        if self.deep_supervision:
        ...
        if self.use_pulask:
            r, u = self.skip2(t2, out); out = out + r; unc.append(u)
        else:
            out = out + t2
        if self.deep_supervision:
        ...
        if self.use_pulask:
            r, u = self.skip1(t1, out); out = out + r; unc.append(u)
        else:
            out = out + t1
        if self.deep_supervision:
        ...
        main_out = [final] + ds if (self.deep_supervision and self.training) else final
